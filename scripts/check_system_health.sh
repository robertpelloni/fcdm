#!/bin/bash
# FCDM System Health Check (v24.1.0)
...
if command -v aplay > /dev/null; then
    ...
    # Priority Auto-Discovery (v24.1.0)
    # Priority order: Teensy -> USB -> Internal -> HDMI/Generic
    ...
    if [ -z "$DETECTED_CARD" ]; then
        DETECTED_CARD=$(echo "$CARDS" | grep -i "HDMI" | head -n 1 | cut -d' ' -f2 | tr -d ':')
    fi
