...
class FSRCalibrator:
    """
    v3.4.0 FCDM Industrial Hardware Diagnostic Utility.
    ...
    def log_burn_in(self, raw_values, strike_count):
        os.makedirs(os.path.dirname(self.log_path), exist_ok=True)
        with open(self.log_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([time.ctime(), strike_count] + raw_values)

    def run(self, burn_in=False):
        print(f"FCDM FSR Utility (v3.4.0) - Mode: {'BURN-IN' if burn_in else 'CALIB'}")
        ...
        try:
            while True:
                ...
                print(f"Strikes: {strike_count} | Profile: {self.active_profile}")
                ...
                if burn_in and strike_count % 100 == 0:
                    self.log_burn_in(raw_values, strike_count)
